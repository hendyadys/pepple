...
FRAME_WIDTH = 496  # Resized frame width
...
# fake the environment position, reward system
def step(position, action, train_data, targets):
    x, y = position
    x_diff, y_diff = _get_pred_coord(action)
    new_x = x + y_diff  # be careful of meaning
    new_y = y + x_diff
...
def _is_terminal(position, train_data):
    x,y = position
    start_coord = start_pos(train_data)
    if
    return False


def _get_pred_coord(action):   # 1- hot vector to direction
    action = np.insert(action, 4, 0)   # insert current position at center
    box = action.reshape((3, 3))
...
def start_pos(coord_data, idx=0):
    start_coords = coord_data[idx,]
    return start_coords
...
def main():
    ...
    agent = Agent(num_actions=8)

    imgs_train, target_train = load_qtrain_data()
    cur_idx = 0     # just take first image

    if TRAIN:  # Train mode
        for _ in range(NUM_EPISODES):
            terminal = False
            # observation = env.reset()     # initialise
            observation = start_pos(cur_idx)
            ...
            while not terminal:
                ...
                observation, reward, terminal, _ = step(observation, action)
                # env.render()
        ...
        for _ in range(NUM_EPISODES_AT_TEST):
            ...
            while not terminal:
                ...
                observation, reward, terminal, _ = step(action)
                # env.render()
                processed_observation = preprocess(observation, last_observation)
