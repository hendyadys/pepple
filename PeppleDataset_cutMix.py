...
# DATA_FOLDER = os.path.join('z:/yue/pepple', 'classification', 'data', '2021.07.07 New TIFFs')
NUM_CLASSES = 6


class PeppleDataset_cutMix(Dataset):
    ...
    def __getitem__(self, idx, visualise=False):
        ...
        rx = int(round(random.random() * W))
        rw = int(round(W * math.sqrt(1-lambda_var)))
        ry = int(round(random.random() * H))
        rh = int(round(H * math.sqrt(1-lambda_var)))
        ...
        # print(idx, img_label, W, H, jdx, img_label_j, rx, rw, ry, rh)
        img_mixed = img.copy()
        img_mixed[min(ry, rh):max(ry, rh), min(rx, rw):max(rx, rw)] = img_j[min(ry, rh):max(ry, rh), min(rx, rw):max(rx, rw)]
...
if __name__ == "__main__":
    ...
    k_ds = PeppleDataset_cutMix(train_img_labels, transform=transform)
    for idx in range(10):
